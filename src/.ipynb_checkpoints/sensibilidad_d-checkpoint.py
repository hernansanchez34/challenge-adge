...
for d in d_values:
    ...
    # Guardar embeddings
    model.wv.save_word2vec_format(f"emb/blogcatalog_d{d}.emb")
...
df_result = pd.DataFrame({
    "d": d_values,
    "log2(d)": np.log2(d_values),
    "Macro-F1": f1_scores
})
df_result.to_csv("archivos_csv/f1_vs_log2d.csv", index=False)

# Graficar
plt.figure(figsize=(8, 5))
plt.plot(np.log2(d_values), f1_scores, marker='o', linestyle='-', color='purple')
plt.title("Macro-F1 vs log₂(d) en BlogCatalog (p=1, q=1)")
plt.xlabel("log₂(d)")
plt.ylabel("Macro-F1")
plt.grid(True)
plt.tight_layout()
plt.savefig("graficas/f1_vs_log2d.png", dpi=300)
print("Gráfico guardado en graficas/f1_vs_log2d.png")
